...
final class GLMaterial implements Material {
	...
	private GLMaterial(String name, String shaderPathV, String shaderPathF) {
		this.name = name;
		this.shaderPathV = shaderPathV;
		this.shaderPathF = shaderPathF;
	}
	
	static GLMaterial loadMaterial(String path) {
		GLMaterial res = cache.get(path);
		if(res != null) return res.clone();
		
		MaterialJsonReader reader = new MaterialJsonReader(path);
		
		res = new GLMaterial(path, reader.vertexShader, reader.fragmentShader);
		...
		for(Entry<String, Texture> e : reader.paramsTex.entrySet())
	...
	public GLMaterial clone() {
		GLMaterial res = new GLMaterial(name, shaderPathV, shaderPathF);
